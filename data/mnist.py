...
def get_training_data_set(num,onerow=False,one_hot=True):
    image_loader = ImageLoader('/Users/xingoo/Documents/dataset/mnist/train-images.idx3-ubyte', num)  # 参数为文件路径和加载的样本数量
    label_loader = LabelLoader('/Users/xingoo/Documents/dataset/mnist/train-labels.idx1-ubyte', num)  # 参数为文件路径和加载的样本数量
...
def get_test_data_set(num,onerow=False,one_hot=True):
    image_loader = ImageLoader('/Users/xingoo/Documents/dataset/mnist/t10k-images.idx3-ubyte', num)  # 参数为文件路径和加载的样本数量
    label_loader = LabelLoader('/Users/xingoo/Documents/dataset/mnist/t10k-labels.idx1-ubyte', num)  # 参数为文件路径和加载的样本数量
    return image_loader.load(onerow), label_loader.load(one_hot)
...
if __name__=="__main__":
    ...
    onepic = train_data_set[index]  # 取一个样本
    printimg(onepic)  # 打印出来这一行所显示的图片
    print(train_labels[index].argmax())  # 打印样本标签